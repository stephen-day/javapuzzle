...
Memory Usage: 38.9 MB, less than 87.47% of Java online submissions for Edit Distance.

This approach appears to work faster in Leet testing than a simple/unintelligent dp recursion.
*/
...
*/
    ...
    public int minDistance(String word1, String word2) {
        ...
        int beginIdx = 0;
        while (beginIdx < lWord.length() && beginIdx < sWord.length() && lWord.charAt(beginIdx) == sWord.charAt(beginIdx))
            beginIdx++;
        ...
        while (lWordEndIdx >= beginIdx && sWordEndIdx >= beginIdx && lWord.charAt(lWordEndIdx) == sWord.charAt(sWordEndIdx)) {
            lWordEndIdx--;
            sWordEndIdx--;
        }
    ...
    private int getOpCountMain(int lBeginIdx, int sBeginIdx, int opCountSoFar) {
        ...
        while (lBeginIdx <= lWordEndIdx && sBeginIdx <= sWordEndIdx && lWord.charAt(lBeginIdx) == sWord.charAt(sBeginIdx)) {
            lBeginIdx++;
            sBeginIdx++;
        }
        ...
        int lNextIdx = nextIdxOf(lWord, sWord.charAt(sBeginIdx), lBeginIdx + 1, lWordLen - (sWordLen / 2) - 1);
        int sNextIdx = nextIdxOf(sWord, lWord.charAt(lBeginIdx), sBeginIdx + 1, sWordLen - (lWordLen / 2) - 1);
        ...
        while (lBeginIdx + replaceCount < lWordEndIdx &&
               ...
               lWord.charAt(lBeginIdx) == lWord.charAt(lBeginIdx + replaceCount) &&
               sWord.charAt(sBeginIdx) == sWord.charAt(sBeginIdx + replaceCount)) {
            replaceCount++;
        }
    ...
    private static int nextIdxOf(String s, char findChar, int startIdx, int limit) {
        if (limit > 0) {
            // Change limit to an endIdx.
            limit += startIdx;

            do {
                if (s.charAt(startIdx) == findChar)
                    return startIdx;
    ...
    private static int countChangesForCloseMatch(String lWord, int lBeginIdx, int lEndIdx,
                                                 String sWord, int sBeginIdx, int sEndIdx) {
        if (sEndIdx == sBeginIdx)
            for (int idx = lBeginIdx + 1; idx <= lEndIdx; idx++)
                if (lWord.charAt(idx) == sWord.charAt(sBeginIdx))
